...
provider "yandex" {
token     = var.yandex_cloud_token # Используйте переменную для токена
...
network_interface {
subnet_id = yandex_vpc_subnet.subnet-1.id # Используйте идентификатор подсети
nat       = true
}
metadata = {
user-data = "${file("./meta.txt")}"
}
depends_on = [yandex_vpc_subnet.subnet-1] # Добавьте зависимость от подсети
}

# Создание таргет-группы и добавление в нее виртуальных машин
resource "yandex_lb_target_group" "mytargetgroup" {
name      = "mytargetgroup"
folder_id = "b1ghu4p1loak83ulpmm5" # Используйте идентификатор каталога
region_id = "ru-central1"

target {
subnet_id = yandex_vpc_subnet.subnet-1.id # Используйте идентификатор подсети
address   = yandex_compute_instance.vmi[0].network_interface.0.ip_address
}
target {
subnet_id = yandex_vpc_subnet.subnet-1.id # Используйте идентификатор подсети
address   = yandex_compute_instance.vmi[1].network_interface.0.ip_address
}
}
